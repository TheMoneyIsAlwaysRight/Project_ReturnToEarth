...
{
    /// <summary>
    /// 노치가 있는 모바일 기기를 위한 SafeAreaAsset 구현. 
    /// 사용법:
    /// (1) 이 컴포넌트를 GUI 패널의 최상단 레벨에 추가하세요.
    /// (2) 패널이 전체 화면 배경 이미지를 사용하는 경우, 이 컴포넌트를 그 패널의 즉시 자식으로 추가하고, 모든 다른 요소들은 그 아래에 자식으로 두세요.
    /// 이렇게 하면 배경 이미지가 노치 뒤의 화면 전체에 걸쳐서 확장되어 더 보기 좋게 보입니다.
    /// (3) 전체 수평 및 수직 배경 줄무늬를 혼합하여 사용하는 다른 경우에는 필요한 요소별로 X 및 Y 적응 컨트롤을 사용하세요.
    /// </summary>
    public class SafeAreaAsset : MonoBehaviour
    {
        #region Simulations
        /// <summary>
